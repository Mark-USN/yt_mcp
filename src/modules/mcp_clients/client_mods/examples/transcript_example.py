...
class DocumentDirProvider(Protocol):
    ...
    def documents_dir(self) -> Path:
        """ Return the base directory where documents (like transcripts) should be stored.
            ...
            different application infos without requiring a full dependency on the app info.
        """
...
class TranscriptExample:
    """ Example of how to use the MCP client to call a tool that retrieves YouTube video
        transcripts and then cache those transcripts to disk.
    """
    mcp_client: Client
    tv: TreeView
    out_path: Path
    ...
    def __init__(
        ...
        """ Initialize the TranscriptExample with an MCP client a document directory provider
            ...
            Side Effects:
                Initializes a TreeView for rendering results and sets up the output path for
                transcripts.
        """
    ...
    async def run(self, video_urls: str | list[str]) -> None:
        ...
        async with self.mcp_client:
            for url in urls:
                ...
                result = await self.mcp_client.call_tool("youtube_json", {"url": url})
